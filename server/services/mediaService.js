...
import { getModeById, STYLE_HINTS } from "../modes.js";
...
const GEMINI_API_KEY_MODE = String(process.env.GEMINI_API_KEY_MODE || "auto").trim().toLowerCase();
...
export function getMediaHealthInfo() {
  return {
...
export function normalizeBatchSeeds(input) {
  return Array.isArray(input)
    ? input.map((item) => String(item || "").trim()).filter(Boolean).slice(0, 20)
    : [];
}

export function sanitizeTaskPayload(input) {
...
export function calcBatchProgress(completedSeeds, totalSeeds, innerProgress = 0) {
...
export async function generateOne(input, options = {}) {
...
export function hasImageSeed(input) {
  const idea = input?.idea && typeof input.idea === "object" ? input.idea : {};
  return Boolean(
...
export function hasVideoSeed(input) {
  const body = input && typeof input === "object" ? input : {};
